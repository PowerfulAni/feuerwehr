	...
	public EinsatzDetails(Gui gui, JTextPane panelMain, Feuerwache feuerwache, int id) {
		...
		panelMain.setPreferredSize(new Dimension(700, 420));
			...
			for (int i = 0; i < fahrzeuge.size(); i++) {
				content += "<tr><td style='text-align: center;'><div style='border: 1px solid black; padding: 5px; margin: 5px;'>" + fahrzeuge.get(i).getKennzeichen() + "</div></td><td><b>"
						+ fahrzeuge.get(i).getFahrzeugName() + "</b> ("+fahrzeuge.get(i).getSitzplaetze()+" Sitzplätze)<hr>" + fahrzeuge.get(i).getFahrzeugTyp() + " | "
						+ fahrzeuge.get(i).getSpezial() + "</td>";
			...
			for (int i = 0; i < mitarbeiter.size(); i++) {
				content += "<tr><td style='text-align: left;'>"+(i+1)+"</td><td style='text-align: left;'>"+mitarbeiter.get(i).getName()+"</td><td style='text-align: left;'>"+mitarbeiter.get(i).getFahrerlaubnis()+"</td></tr>";
