...
public class FeuerwehrleuteView extends JScrollPane {
	...
	public FeuerwehrleuteView(JTextPane panelMain, Feuerwache feuerwache) {
		...
		for (int i = 0; i < feuerwehrleute.size(); i++) {
			content += "<tr style='text-align: center; border-bottom: 1px dotted black;'><td>"+((int)i+1)+"</td><td style='text-align: left;'>"+((Feuerwehrmensch) feuerwehrleute.get(i)).getName()+"</td><td>"+((Feuerwehrmensch) feuerwehrleute.get(i)).getFahrerlaubnis()+"</td><td><form action='#'><input type='submit' value='Speichern'/><select name='status'>";
			content += "<option value='bereit_"+i+"' "+(feuerwehrleute.get(i).getMitarbeiterStatus().equals(MitarbeiterStatus.Bereit) ? "selected" : "" )+">Bereit</option>";
			content += "<option value='einsatz_"+i+"' "+(feuerwehrleute.get(i).getMitarbeiterStatus().equals(MitarbeiterStatus.Einsatz) ? "selected" : "" )+">Einsatz</option>";
			content += "<option value='urlaub_"+i+"' "+(feuerwehrleute.get(i).getMitarbeiterStatus().equals(MitarbeiterStatus.Urlaub) ? "selected" : "" )+">Urlaub</option>";
			content += "<option value='krank_"+i+"' "+(feuerwehrleute.get(i).getMitarbeiterStatus().equals(MitarbeiterStatus.Krank) ? "selected" : "" )+">Krank</option></select></form></td></tr>";
		...
		panelMain.addHyperlinkListener(new HyperlinkListener() {
			...
			public void hyperlinkUpdate(HyperlinkEvent e) {
				if (e instanceof FormSubmitEvent)
                {
                    ...
                    System.out.println(Integer.parseInt(splits[1]));
